...
/* localtify 0.4.1 V424 — Windows startup white-screen recovery. */
...
const { createMediaServerRuntime } = require("./runtime/media-server.cjs");
...
const { readLocalAudioMetadata } = require("./metadata-service.cjs");
const { findFolderCover, resolveSongCover } = require("./cover-service.cjs");
...
const mediaServerRuntime = createMediaServerRuntime({
  protocol,
  net,
  getFileInfoCached,
  fileExists
});
const startLocaltifyMediaServer = mediaServerRuntime.start;
const stopLocaltifyMediaServer = mediaServerRuntime.stop;
const registerLocaltifyMediaProtocol = mediaServerRuntime.registerProtocol;
const safeMediaUrl = mediaServerRuntime.safeMediaUrl;
const getMediaFileVersion = mediaServerRuntime.getMediaFileVersion;
const MEDIA_TOKEN_TTL_MS = mediaServerRuntime.tokenTtlMs;
...
function updateTrayMenu() {
  ...
  try {
    ...
    tray.setContextMenu(Menu.buildFromTemplate([
      ...
      { type: "separator" },
      { label: nativeMediaState.isPlaying ? "Pause" : "Play", enabled: nativeMediaState.hasSong, click: () => sendPlayerCommand({ type: "toggle" }) },
      ...
      { label: nativeMediaState.muted || nativeMediaState.volume <= 0.01 ? "Unmute" : "Mute", enabled: nativeMediaState.hasSong, click: () => sendPlayerCommand({ type: "muteToggle" }) },
      { type: "separator" },
      { label: "Quit", click: () => { allowQuit = true; app.quit(); } }
    ]));
  } catch (error) {
...
function updateNativeMediaState(payload = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(payload, "minimizeToTray")) minimizeToTray = Boolean(payload.minimizeToTray);
  updateTrayMenu();
...
function setupNativeWindowsMediaIpc() {
  ...
  ipcRouter.handle("localitfy:set-start-with-windows", async (_event, payload = {}) => {
    ...
    try { saveSettings({ startWithWindows: Boolean(enabled) }); } catch (error) { console.log("[localitfy startup setting save error]", error?.message || error); }
...
function appendChromiumSwitchOnce(name, value) {
  try {
    ...
    if (typeof app.commandLine?.hasSwitch === "function" && app.commandLine.hasSwitch(name)) return false;
    if (typeof value === "undefined" || value === null || value === "") app.commandLine.appendSwitch(name);
    else app.commandLine.appendSwitch(name, String(value));
    return true;
  } catch (error) {
    console.log("[localitfy chromium switch error]", name, error?.message || error);
    return false;
  }
}
...
function configureLocaltifyChromiumPerformance() {
  ...
  const add = (name, value) => {
    ...
    switches.push({ name, value: typeof value === "undefined" || value === null ? "" : String(value), applied });
    return applied;
  };
  ...
  if (!disableGpuTuning) {
    ...
    if (forceOpenGl) add("use-gl", "desktop");
    else if (process.platform === "win32") add("use-angle", forceAngleGl ? "gl" : "d3d11");
  }
  ...
  if (process.platform === "linux") {
    ...
    if (app.isPackaged && !linuxGpuOptIn) add("disable-accelerated-video-decode");
  }

  return { gpuTuningEnabled: !disableGpuTuning, forceOpenGl, forceAngleGl, platform: process.platform, switches };
}

app.setName(APP_NAME);
const LOCALTIFY_CHROMIUM_PERFORMANCE = configureLocaltifyChromiumPerformance();
try { Menu.setApplicationMenu(null); } catch (error) { console.log("[localtify menu cleanup error]", error?.message || error); }
...
function getCoverThumbnailCacheKey(filePath) {
  try {
    ...
    const signature = [path.normalize(String(filePath || "")).toLowerCase(), info.size || 0, Math.round(info.mtimeMs || 0), COVER_THUMBNAIL_SIZE].join("|");
...
function createCoverThumbnailSync(filePath) {
  ...
  if (!thumbnailPath) return { ok: false, sourcePath: filePath, thumbnailPath: "", error: "That cover image is missing or unsupported." };
  if (fileExists(thumbnailPath)) return { ok: true, sourcePath: filePath, thumbnailPath, cached: true, url: safeMediaUrl(thumbnailPath) };

  try {
    const image = nativeImage.createFromPath(filePath);
    if (!image || image.isEmpty()) return { ok: false, sourcePath: filePath, thumbnailPath: "", error: "Localtify could not read this cover image. The original cover will still be used." };
    const sourceSize = image.getSize();
    const resized = image.resize({ width: COVER_THUMBNAIL_SIZE, height: COVER_THUMBNAIL_SIZE, quality: "good" });
    if (!resized || resized.isEmpty()) return { ok: false, sourcePath: filePath, thumbnailPath: "", error: "Localtify could not resize this cover image. The original cover will still be used." };
    fs.mkdirSync(path.dirname(thumbnailPath), { recursive: true });
    fs.writeFileSync(thumbnailPath, resized.toPNG());
    return { ok: true, sourcePath: filePath, thumbnailPath, cached: false, url: safeMediaUrl(thumbnailPath), sourceWidth: sourceSize.width, sourceHeight: sourceSize.height, size: COVER_THUMBNAIL_SIZE };
  } catch (error) {
    console.log("[localtify thumbnail create error]", error?.message || error);
    return { ok: false, sourcePath: filePath, thumbnailPath: "", error: "Localtify could not create a small cover thumbnail yet. The full cover will still work." };
  }
}
...
function getCoverThumbnailUrl(filePath, options = {}) {
  ...
  if (cached && fileExists(cached.thumbnailPath)) return cached.url;
  const thumbnailPath = getCoverThumbnailPath(filePath);
...
function getCoverThumbnailStatus() {
  ...
  try {
    ...
    return { ok: false, directory: dir, count, sizeBytes, queued: coverThumbnailQueue.size, message: "Localtify could not read the thumbnail cache. Covers will still load normally.", error: error?.message || "thumbnail status failed" };
  }
  return { ok: true, directory: dir, count, sizeBytes, queued: coverThumbnailQueue.size, size: COVER_THUMBNAIL_SIZE, message: count ? `${count} small cover thumbnail${count === 1 ? "" : "s"} cached.` : "No cover thumbnails cached yet. Localtify will build them quietly as covers appear." };
}
...
function cleanupCoverThumbnailCache() {
  ...
  let sizeBytes = 0;
  try {
    for (const timer of coverThumbnailQueue.values()) { try { clearTimeout(timer); } catch {} }
    ...
    clearFileInfoCache();
    return { ok: true, directory: dir, removed, sizeBytes, message: removed ? `Cleaned ${removed} cached cover thumbnail${removed === 1 ? "" : "s"}.` : "Cover cache is already clean." };
  } catch (error) {
    console.log("[localtify cover cache cleanup error]", error?.message || error);
    return { ok: false, directory: dir, removed, sizeBytes, error: error?.message || "cover cache cleanup failed", message: "Localtify could not clean the cover cache right now." };
  }
}
...
function warmCoverThumbnails({ limit = 80, force = false } = {}) {
  ...
  for (const coverPath of uniqueCoverPaths) {
    ...
    if (thumbnailPath && fileExists(thumbnailPath) && !force) { cached += 1; continue; }
  ...
  return { ok: true, scanned: uniqueCoverPaths.length, created, cached, warnings: Array.from(new Set(warnings)).slice(0, 4), status: getCoverThumbnailStatus(), message: created ? `Cached ${created} cover thumbnail${created === 1 ? "" : "s"}.` : cached ? "Cover thumbnails are already ready." : "No custom covers needed thumbnail caching yet." };
}
...
function scheduleCoverThumbnailWarmup() {
  ...
  setTimeout(() => {
    try { warmCoverThumbnails({ limit: 120 }); } catch (error) { console.log("[localtify thumbnail warmup error]", error?.message || error); }
  }, 4500);
}
...
function downloadHttpsBuffer(urlString, maxBytes = 8 * 1024 * 1024, redirectLimit = 4) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on("data", (chunk) => {
        total += chunk.length;
        if (total > maxBytes) { req.destroy(new Error("cover image too large")); return; }
        chunks.push(chunk);
      });
...
function getFfmpegPath() {
  ...
  if (app.isPackaged && ffmpegStatic.includes("app.asar")) return ffmpegStatic.replace("app.asar", "app.asar.unpacked");
  return ffmpegStatic;
}
...
const SPOTIFY_REDIRECT_URI = String(process.env.SPOTIFY_REDIRECT_URI || `http://127.0.0.1:${SPOTIFY_REDIRECT_PORT}/spotify/callback`);
...
function makeSpotifyCodeVerifier() { return base64UrlEncode(crypto.randomBytes(48)); }
function makeSpotifyCodeChallenge(verifier) { return base64UrlEncode(crypto.createHash("sha256").update(verifier).digest()); }
function readSpotifyOAuthToken() {
  try {
    ...
    return { accessToken: typeof parsed.accessToken === "string" ? parsed.accessToken : "", refreshToken: typeof parsed.refreshToken === "string" ? parsed.refreshToken : "", expiresAt: Number(parsed.expiresAt || 0) };
  } catch { return null; }
}
function writeSpotifyOAuthToken(token) {
  ...
  } catch (error) { console.log("[localitfy spotify token save error]", error?.message || error); }
}
function clearSpotifyOAuthToken() {
  _spotifyToken = null;
  _spotifyTokenExpiry = 0;
  try { fs.unlinkSync(SPOTIFY_OAUTH_FILE()); } catch {}
}
...
async function exchangeSpotifyAuthorizationCode(code, codeVerifier) {
  const data = await spotifyTokenRequest({ grant_type: "authorization_code", code, redirect_uri: SPOTIFY_REDIRECT_URI, client_id: SPOTIFY_CLIENT_ID, code_verifier: codeVerifier });
...
async function refreshSpotifyAccessToken(previous = readSpotifyOAuthToken()) {
  ...
  const data = await spotifyTokenRequest({ grant_type: "refresh_token", refresh_token: previous.refreshToken, client_id: SPOTIFY_CLIENT_ID });
  return saveSpotifyOAuthResponse(data, previous);
}
async function getSpotifyAccessToken() {
  if (!SPOTIFY_CLIENT_ID) throw new Error("Spotify OAuth is unavailable, but public fallback can still fetch public Spotify links.");
  const now = Date.now();
  ...
  if (saved?.refreshToken) return (await refreshSpotifyAccessToken(saved)).accessToken;
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    server = http.createServer((req, res) => {
      try {
        ...
        finish(null, { code });
      } catch (error) { if (typeof finish === "function") finish(error); }
    });
    server.once("error", (error) => { if (typeof finish === "function") finish(new Error(`Spotify callback server failed: ${error?.message || error}`)); });
    timer = setTimeout(() => { if (typeof finish === "function") finish(new Error("Spotify login timed out.")); }, 5 * 60 * 1000);
    server.listen(SPOTIFY_REDIRECT_PORT, "127.0.0.1");
  });
  return { promise, cancel: (reason = "Spotify login cancelled.") => { if (!settled && typeof finish === "function") finish(new Error(reason)); } };
}

async function loginSpotifyOAuth() {
  if (!SPOTIFY_CLIENT_ID) {
    return { ok: true, ready: true, loggedIn: false, publicOnly: true, fallbackAvailable: true, mode: "public-fallback", needsClientId: false, redirectUri: SPOTIFY_REDIRECT_URI, error: "Spotify OAuth is unavailable, but public playlist/album/track import is ready." };
  }
  ...
  try {
    ...
    return { ok: true, ready: true, loggedIn: true, publicOnly: true, mode: "oauth-pkce", redirectUri: SPOTIFY_REDIRECT_URI, message: "Spotify connected. Public playlists, albums, and tracks can be imported." };
  } catch (error) {
    const message = error?.message || "Spotify login failed.";
    return { ok: false, ready: Boolean(SPOTIFY_CLIENT_ID), loggedIn: false, cancelled: /cancel/i.test(message), publicOnly: true, mode: "oauth-pkce", redirectUri: SPOTIFY_REDIRECT_URI, error: message };
  } finally {
    waiter.cancel();
    try { if (loginWindow && !loginWindow.isDestroyed()) loginWindow.close(); } catch {}
  }
...
async function spotifyApiGet(endpoint) {
  ...
  const fetchOnce = async () => spotifyHttpGet(url, { Authorization: `Bearer ${await getSpotifyAccessToken()}`, Accept: "application/json" });
  ...
  if (res.status === 403 || res.status === 404) throw new Error("Spotify could not read this link. Make sure the playlist is public, then copy the link again.");
...
function shapeSpotifyTrack(track, fallbackAlbumName = "", fallbackCoverUrl = "") {
  ...
  const coverUrl = bestSpotifyImage(track.album?.images) || bestSpotifyImage(track.images) || String(track.coverUrl || track.spotifyCoverUrl || track.albumCoverUrl || fallbackCoverUrl || "");
  return {
...
async function fetchSpotifyOembed(trackUrl) {
  const res = await spotifyHttpGet(`https://open.spotify.com/oembed?url=${encodeURIComponent(trackUrl)}`, { Accept: "application/json" });
  try {
    mainWindow.setBackgroundColor("#000000");
    if (process.platform === "win32" && typeof mainWindow.setBackgroundMaterial === "function") mainWindow.setBackgroundMaterial("none");
  } catch {}
  try { mainWindow.setBackgroundColor("#000000"); } catch {}
...
async function fetchSpotifyPublicTrackFallback(trackId) {
  ...
  return { id: trackId, title, name: title, artist, artists: artist, albumName: "", coverUrl, albumCoverUrl: coverUrl, spotifyCoverUrl: coverUrl, duration: 0, durationMs: 0, spotifyUrl: trackUrl };
}
...
async function fetchSpotifyPublicPlaylistFallback(playlistId) {
  ...
  for (const url of urlsToTry) {
    try {
      ...
      if (ids.length) break;
    } catch (error) { console.log("[localtify spotify fallback page error]", error?.message || error); }
  }
  const trackIds = uniqSpotifyIds(ids);
  if (!trackIds.length) throw new Error("Spotify could not expose tracks for this playlist. Make sure it is public on your profile, not only shareable by link, then try again.");
  const tracks = [];
  const limitedIds = trackIds.slice(0, 250);
  for (let index = 0; index < limitedIds.length; index += 8) {
    const shaped = await Promise.all(limitedIds.slice(index, index + 8).map((trackId) => fetchSpotifyPublicTrackFallback(trackId).catch(() => null)));
...
async function fetchSpotifyPublicAlbumFallback(albumId) {
  ...
  for (let index = 0; index < trackIds.length; index += 8) {
    const shaped = await Promise.all(trackIds.slice(index, index + 8).map((trackId) => fetchSpotifyPublicTrackFallback(trackId).catch(() => null)));
...
async function fetchSpotifyPublicFallback(rawUrl, reason = "") {
  ...
  if (parsed.type === "album") return fetchSpotifyPublicAlbumFallback(parsed.id);
  if (parsed.type === "playlist") return fetchSpotifyPublicPlaylistFallback(parsed.id);
...
async function fetchSpotifyTracksFromUrl(rawUrl) {
  const parsed = parseSpotifyUrl(rawUrl);
  if (!parsed) throw new Error("Paste a valid Spotify playlist, album, or track link.");
  if (!SPOTIFY_CLIENT_ID) return fetchSpotifyPublicFallback(rawUrl, "spotify client id unavailable");
  ...
  try {
    ...
    if (type === "playlist") {
      ...
      if (info && info.public === false) throw new Error("This playlist is link-shareable but not public to the Spotify API. Add it to your public profile, or try the fallback fetch again.");
      const tracks = [];
      let offset = 0;
      while (true) {
...
function getPixelArtDirectory(options = {}) {
  ...
  const now = Date.now();
  if (!force && pixelArtDirectoryCache.value && pixelArtDirectoryCache.expiresAt > now) return pixelArtDirectoryCache.value;
  try {
    ...
    for (const candidatePath of candidates) {
      try {
        ...
        if (hasImages) {
          if (pixelArtDirectoryCache.value !== candidatePath) console.log(`[localitfy pixelart] using ${candidatePath}`);
    ...
    if (pixelArtDirectoryCache.value !== userDataPixelDir) console.log(`[localitfy pixelart] no bundled pixelart found, using empty user folder ${userDataPixelDir}`);
...
function getPixelArtFiles(options = {}) {
  ...
  if (!root) return [];
  if (!force && pixelArtFilesCache.root === root && pixelArtFilesCache.expiresAt > now) return pixelArtFilesCache.value;
  try {
    const files = fs.readdirSync(root).map((name) => path.join(root, name)).filter(fileExists).filter(isImageFile);
...
function listPixelPacksDetailed() {
  ...
  } catch (error) { console.log("[localitfy pixel packs error]", error?.message || error); }
  return packs;
}
...
function getPixelArtFilesFromPack(packName = "default") {
  ...
  try { return fs.readdirSync(packPath).map((name) => path.join(packPath, name)).filter(fileExists).filter(isImageFile); } catch { return []; }
}

function listPixelCoversDetailed() {
  const songs = getSongs();
  const usage = new Map();
  for (const song of songs) if (song.coverPath) usage.set(song.coverPath, (usage.get(song.coverPath) || 0) + 1);
  return getPixelArtFiles().map((coverPath) => ({ name: path.basename(coverPath), key: path.parse(coverPath).name, path: coverPath, url: safeMediaUrl(coverPath), usageCount: usage.get(coverPath) || 0, exists: fileExists(coverPath), broken: !fileExists(coverPath) }));
}

function getCoverStats() {
  const songs = getSongs();
  const covers = listPixelCoversDetailed();
  const brokenSongs = songs.filter((song) => song.coverPath && !fileExists(song.coverPath)).map((song) => ({ id: song.id, title: song.title, coverPath: song.coverPath }));
  return { pixelDir: getPixelArtDirectory(), coverCount: covers.length, songCount: songs.length, songsWithCovers: songs.filter((song) => Boolean(song.coverPath)).length, songsMissingCovers: songs.filter((song) => !song.coverPath).length, songsWithBrokenCovers: brokenSongs.length, brokenItems: brokenSongs };
}
...
function pickLeastUsedCover(availableCovers = [], fallbackSongs = [], choices = {}) {
  ...
  for (const song of songs) if (song.coverPath && usage.has(song.coverPath)) usage.set(song.coverPath, usage.get(song.coverPath) + 1);
  let candidates = [...availableCovers];
  if (choices.avoidSelectedCurrent && choices.currentPath) candidates = candidates.filter((p) => p !== choices.currentPath);
...
async function makeSongFromFile(filePath, pixelArtFiles = [], usedCovers = new Set(), options = {}) {
  ...
  const coverResult = await resolveSongCover({
    song: { id, title, artist, album, filePath, coverPath: options.coverPath || "", coverSource: options.coverSource || "" },
    metadata,
    filePath,
  ...
  if (coverResult.coverSource === "fallback" && coverResult.coverPath) usedCovers.add(coverResult.coverPath);
  ...
  return { id, title, artist, album, filePath, coverPath: coverResult.coverPath || "", coverSource: coverResult.coverSource || "none", coverUpdatedAt: coverResult.coverUpdatedAt || new Date().toISOString(), duration, durationMs: durationMs || (duration > 0 ? duration * 1000 : 0), track: Number(metadata?.track || 0) || 0, disc: Number(metadata?.disc || 0) || 0, year: metadata?.year || "", bitrate: 0, addedAt: Date.now() };
}

function normalizeForLooseMatch(value = "") {
  return String(value || "").toLowerCase().replace(/\.[a-z0-9]+$/i, "").replace(/\([^)]*\)|\[[^\]]*\]/g, " ").replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ").trim();
}
...
function listAudioFilesInDirectory(directory, options = {}) {
  ...
  const walk = (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      if (!entry.isFile() || !isAudioFile(fullPath) || /\.(part|tmp|download|crdownload)$/i.test(entry.name)) continue;
      out.push(fullPath);
    }
  };
  walk(root, 0);
  out.sort((a, b) => (getFileInfoCached(b).mtimeMs || 0) - (getFileInfoCached(a).mtimeMs || 0));
  return out;
}
...
function matchSpotifyTrackForFile(filePath, tracks = []) {
  ...
  for (const track of tracks) {
    ...
    let overlap = 0;
    for (const word of trackWords) if (fileWords.has(word)) overlap += 1;
    score += overlap;
...
async function makeSongFromFileWithMetadata(filePath, tracks = [], pixelArtFiles = [], usedCovers = new Set()) {
  ...
  return { ...baseSong, title: title || baseSong.title, artist: artist || baseSong.artist, album: album || baseSong.album || "", coverPath: useSpotifyCover ? cachedCoverPath : baseSong.coverPath, coverSource: useSpotifyCover ? "spotify" : baseSong.coverSource, coverUpdatedAt: useSpotifyCover ? new Date().toISOString() : baseSong.coverUpdatedAt, duration: Number(matched?.duration || Math.round(Number(matched?.durationMs || 0) / 1000) || baseSong.duration || 0), durationMs: Number(matched?.durationMs || baseSong.durationMs || 0) || 0 };
}
...
async function makeSongFromFileWithExactSpotifyTrack(filePath, track = {}, pixelArtFiles = [], usedCovers = new Set(), sourceInfo = {}) {
  ...
  const useSpotifyCover = cachedCoverPath && (!baseSong.coverPath || baseSong.coverSource === "fallback" || baseSong.coverSource === "none");
  return { ...baseSong, title: title || baseSong.title, artist: artist || baseSong.artist, album: album || baseSong.album || "", coverPath: useSpotifyCover ? cachedCoverPath : baseSong.coverPath, coverSource: useSpotifyCover ? "spotify" : baseSong.coverSource, coverUpdatedAt: useSpotifyCover ? new Date().toISOString() : baseSong.coverUpdatedAt, duration, durationMs: Number(track?.durationMs || duration * 1000 || baseSong.durationMs || 0) || 0, sourceType: "spotify", sourceTrackId: spotifyTrackId, sourceUrl: spotifyUrl, sourceProvider: String(sourceInfo?.sourceProvider || sourceInfo?.provider || "youtube"), sourceProviderUrl: String(sourceInfo?.sourceProviderUrl || sourceInfo?.providerUrl || ""), sourceMatchScore: Number(sourceInfo?.sourceMatchScore ?? sourceInfo?.matchScore ?? 0) || 0 };
}
...
async function importNewAudioFilesFromDirectory(directory, tracks = [], options = {}) {
  ...
  if (!audioFiles.length) return { changedCount: 0, importedCount: 0, songs: listSongsShaped(), files: [] };
  const existingPaths = new Set(getSongs().map((song) => path.normalize(String(song.filePath || "")).toLowerCase()).filter(Boolean));
  ...
  if (!newFiles.length) return { changedCount: 0, importedCount: 0, songs: listSongsShaped(), files: [] };
  const pixelArtFiles = getPixelArtFiles();
  const usedCovers = new Set();
  const importedSongs = [];
  for (const filePath of newFiles) importedSongs.push(await makeSongFromFileWithMetadata(filePath, tracks, pixelArtFiles, usedCovers));
  const changedCount = insertSongs(importedSongs);
  clearFileInfoCache();
  console.log("[localtify library refresh] imported new audio files from folder", { folder: directory, found: audioFiles.length, newFiles: newFiles.length, changedCount });
  return { changedCount, importedCount: importedSongs.length, songs: listSongsShaped(), files: newFiles };
}
...
function cleanupAlbumTrackName(value = "") {
  return cleanAlbumFolderText(value).replace(/^\s*(?:cd|disc)?\s*\d{1,2}\s*[-_. ]+\s*/i, "").replace(/^\s*\d{1,3}\s*[-_. ]+\s*/i, "").replace(/^\s*\d{1,2}-\d{1,3}\s*[-_. ]+\s*/i, "").trim();
}
function parseAlbumTrackFileName(filePath, fallbackAlbumTitle = "local album") {
  ...
  if (parts.length >= 2) { artist = parts[0] || artist; title = parts.slice(1).join(" - ") || title; }
  return { title: cleanAlbumFolderText(title, parsed.name || "track"), artist: cleanAlbumFolderText(artist, "unknown artist"), album: cleanAlbumFolderText(fallbackAlbumTitle, "local album") };
}
...
function findAlbumFolderCoverPath(folderPath) { return findFolderCover(folderPath); }
function folderHasAudioFiles(folderPath) { return listAudioFilesInDirectory(folderPath, { maxDepth: 0, maxFiles: 8 }).length > 0; }
function isDiscLikeAlbumSubfolderName(name = "") { return ALBUM_FOLDER_DISC_FOLDER_RE.test(String(name || "").trim()); }
function folderHasDiscAudioFiles(folderPath) {
  let entries = [];
  try { entries = fs.readdirSync(folderPath, { withFileTypes: true }); } catch { return false; }
  return entries.some((entry) => entry.isDirectory() && isDiscLikeAlbumSubfolderName(entry.name) && folderHasAudioFiles(path.join(folderPath, entry.name)));
}
function isAlbumFolderCandidate(folderPath) { return folderHasAudioFiles(folderPath) || folderHasDiscAudioFiles(folderPath); }
function isIgnoredAlbumLibraryFolderName(name = "") { return String(name || "").startsWith(".") || name === "localitfy-bin" || name === "spotify-covers" || name === "node_modules" || name === "release" || name === "dist"; }
function findAlbumFoldersFromRoot(rootPath, mode = "single") {
  ...
  while (queue.length && folders.length < ALBUM_FOLDER_MAX_ALBUMS) {
    ...
    if (isAlbumFolderCandidate(folderPath)) { folders.push(folderPath); continue; }
    ...
    for (const entry of entries) {
      if (!entry.isDirectory() || isIgnoredAlbumLibraryFolderName(entry.name)) continue;
...
async function buildAlbumFolderPreview(folderPath, index = 0) {
  ...
  for (let trackIndex = 0; trackIndex < audioFiles.length; trackIndex += 1) {
    if (trackIndex > 0 && trackIndex % ALBUM_FOLDER_SCAN_YIELD_EVERY_TRACKS === 0) await yieldToMainLoop();
    ...
    if (!albumEmbeddedCoverPath && tagMetadata?.embeddedCoverPath && fileExists(tagMetadata.embeddedCoverPath)) albumEmbeddedCoverPath = tagMetadata.embeddedCoverPath;
    const durationMs = Math.max(0, Number(tagMetadata?.durationMs || 0), Number(tagMetadata?.duration || 0) > 0 ? Number(tagMetadata.duration) * 1000 : 0);
    parsedTracks.push({
...
function pruneAlbumFolderScanCache() {
  const now = Date.now();
  for (const [scanId, scan] of albumFolderScanCache) if (!scan?.createdAt || now - scan.createdAt > ALBUM_FOLDER_SCAN_TTL_MS) albumFolderScanCache.delete(scanId);
}
...
async function handleAlbumFolderScan(event, payload = {}) {
  ...
  if (result.canceled || !result.filePaths?.[0]) return { ok: true, canceled: true, mode, albums: [], albumCount: 0, trackCount: 0, duplicateCount: 0, message: "album folder picker cancelled" };
  ...
  for (let index = 0; index < folderPaths.length; index += 1) {
    ...
    event.sender.send("album-folder-import:progress", { type: "scan-progress", mode, index: index + 1, total: folderPaths.length, folder: folderPath, folderPath, message: `Scanning ${path.basename(folderPath)}` });
  ...
  const scanPayload = { ok: true, canceled: false, scanId, mode, rootPath, albumCount: albums.length, trackCount, duplicateCount, albums, message: albums.length ? `Found ${albums.length} album${albums.length === 1 ? "" : "s"} with ${trackCount} track${trackCount === 1 ? "" : "s"}.` : "No audio files were found in that folder." };
...
async function handleAlbumFolderImport(event, payload = {}) {
  ...
  if (!scan) return { ok: false, error: "Album scan expired. Please choose the album folder again.", songs: listSongsShaped(), albums: [] };
  ...
  event.sender.send("album-folder-import:progress", { type: "import-start", index: 0, total: allTracks.length, message: "Adding album tracks to Localtify..." });
  const existingSongsBefore = getSongs();
  const existingSongByPath = new Map(existingSongsBefore.filter((song) => song?.filePath).map((song) => [path.normalize(String(song.filePath || "")).toLowerCase(), song]));
  ...
  for (let index = 0; index < allTracks.length; index += 1) {
    ...
    if (index === 0 || index + 1 === allTracks.length || index % ALBUM_FOLDER_IMPORT_PROGRESS_EVERY_TRACKS === 0) {
      event.sender.send("album-folder-import:progress", { type: "import-progress", index: index + 1, total: allTracks.length, filePath, message: `Adding ${track.title || path.basename(filePath)}` });
    }
    if (index > 0 && index % ALBUM_FOLDER_IMPORT_YIELD_EVERY_TRACKS === 0) await yieldToMainLoop();
    if (!filePath || !fileExists(filePath) || !isAudioFile(filePath)) continue;
    const folderCoverPath = album.coverSource === "folder" ? album.coverPath || album.folderCoverPath || "" : album.folderCoverPath || "";
    const albumEmbeddedCoverPath = album.coverSource === "embedded" ? album.coverPath || album.embeddedCoverPath || "" : album.embeddedCoverPath || "";
    ...
    if (existingSong) {
      ...
      const coverResult = await resolveSongCover({ song: existingSong, metadata, filePath, folderCoverPath, albumEmbeddedCoverPath, preferFolderCover: Boolean(folderCoverPath), fallbackCoverPath: fallbackCover });
      const durationMs = Math.max(0, Number(track.durationMs || 0), Number(metadata?.durationMs || 0), Number(track.duration || 0) > 0 ? Number(track.duration) * 1000 : 0, Number(metadata?.duration || 0) > 0 ? Number(metadata.duration) * 1000 : 0);
      const duration = durationMs > 0 ? Math.round(durationMs / 1000) : Math.max(0, Number(track.duration || 0), Number(metadata?.duration || 0), Number(existingSong.duration || 0));
      const patch = {
      ...
      if (durationMs > 0) { patch.duration = duration; patch.durationMs = durationMs; }
    ...
    const baseSong = await makeSongFromFile(filePath, pixelArtFiles, usedCovers, { album: album.title || track.album || "", folderCoverPath, albumEmbeddedCoverPath, preferFolderCover: Boolean(folderCoverPath) });
    const durationMs = Math.max(0, Number(track.durationMs || 0), Number(baseSong.durationMs || 0), Number(track.duration || 0) > 0 ? Number(track.duration) * 1000 : 0, Number(baseSong.duration || 0) > 0 ? Number(baseSong.duration) * 1000 : 0);
    const duration = durationMs > 0 ? Math.round(durationMs / 1000) : Math.max(0, Number(track.duration || 0), Number(baseSong.duration || 0));
    importedSongs.push({ ...baseSong, title: cleanAlbumFolderText(track.title, baseSong.title || "track"), artist: cleanAlbumFolderText(track.artist, baseSong.artist || album.artist || "unknown artist"), album: cleanAlbumFolderText(album.title || track.album, baseSong.album || "local album"), coverPath: baseSong.coverPath || "", coverSource: baseSong.coverSource || "none", coverUpdatedAt: baseSong.coverUpdatedAt || new Date().toISOString(), duration, durationMs: durationMs || (duration > 0 ? duration * 1000 : 0), sourceType: "local", sourceProvider: "album-folder", sourceUrl: album.sourcePath || "", sourceProviderUrl: album.sourcePath || "" });
  ...
  const importedAlbums = albums.map((album) => {
    const songIds = (album.tracks || []).map((track) => songByPath.get(path.normalize(String(track.filePath || "")).toLowerCase())?.id).filter(Boolean);
    const albumSongs = songIds.map((id) => songById.get(id)).filter(Boolean);
    const preferredCoverSong = albumSongs.find((song) => String(song.coverSource || "") === "folder" && song.coverPath) || albumSongs.find((song) => String(song.coverSource || "") === "embedded" && song.coverPath) || albumSongs.find((song) => String(song.coverSource || "") === "custom" && song.coverPath) || albumSongs.find((song) => song.coverPath) || null;
    ...
    return { id: album.id, manualAlbumId: crypto.createHash("sha1").update(String(album.sourcePath || album.id || album.title)).digest("hex"), title: album.title, artist: album.artist || "various artists", year: "", coverPath: stableCoverPath, coverSource: stableCoverSource, coverUrl: stableCoverUrl || "", sourceType: "folder", sourcePath: album.sourcePath || "", folderCoverPath: album.folderCoverPath || (album.coverSource === "folder" ? album.coverPath || "" : ""), embeddedCoverPath: album.embeddedCoverPath || (album.coverSource === "embedded" ? album.coverPath || "" : ""), importedAt: importStartedAt, createdAt: importStartedAt, updatedAt: Date.now(), songIds };
  ...
  event.sender.send("album-folder-import:progress", { type: "import-done", index: allTracks.length, total: allTracks.length, changedCount, message });
  albumFolderScanCache.delete(scanId);
  return { ok: true, changedCount, insertedCount, repairedExistingCount, repairedCoverCount, repairedDurationCount, importedCount: importedSongs.length, trackCount: allTracks.length, mode: scan.mode, rootPath: scan.rootPath || "", songs: afterSongs, albums: importedAlbums, message };
}

async function repairSpotifyMetadataForFolder(directory, tracks = [], options = {}) {
  const audioFiles = listAudioFilesInDirectory(directory, options);
  if (!audioFiles.length || !Array.isArray(tracks) || !tracks.length) return { changedCount: 0, songs: listSongsShaped() };
  const songs = getSongs();
  const byPath = new Map(songs.filter((song) => song?.filePath).map((song) => [path.normalize(String(song.filePath)).toLowerCase(), song]));
  let changedCount = 0;
  for (const filePath of audioFiles) {
    ...
    if (title && (!song.title || currentTitle === "unknown title" || currentTitle === normalizeForLooseMatch(path.parse(filePath).name))) patch.title = title;
    if (artist && (!song.artist || currentArtist === "unknown artist" || currentArtist === "unknown" || currentArtist === "slowed" || currentArtist === "slow" || currentArtist === "reverb" || currentArtist === "nightcore" || currentArtist === "sped up")) patch.artist = artist;
    if (album && (!song.album || String(song.album).trim().toLowerCase() === "unknown album")) patch.album = album;
    if (cachedCoverPath) {
      ...
      if (!currentCover || !fileExists(currentCover) || isPixelFallback || !isSpotifyCover) patch.coverPath = cachedCoverPath;
    }
    const duration = Number(matched?.duration || Math.round(Number(matched?.durationMs || 0) / 1000) || 0);
    if (duration > 0 && !Number(song.duration || 0)) patch.duration = duration;
    if (Object.keys(patch).length) {
      try { patchSong(song.id, patch); changedCount += 1; } catch (error) { console.log("[localitfy spotify metadata repair error]", error?.message || error); }
    }
  }
  if (changedCount) clearFileInfoCache();
  console.log("[localtify spotify metadata repair]", { folder: directory, checked: audioFiles.length, repaired: changedCount });
...
function pickStablePixelCoverForSong(song, pixelArtFiles = null) {
  ...
  const seed = [song?.id, song?.title, song?.artist, song?.album, song?.filePath].filter(Boolean).join("::");
  try {
    const digest = crypto.createHash("sha1").update(seed || "localtify-cover").digest();
    return covers[digest.readUInt32BE(0) % covers.length] || "";
  } catch { return covers[0] || ""; }
}
...
function shapeSong(song, pixelArtFiles = null) {
  ...
  return { ...song, url: "", coverPath: savedCoverPath || runtimeCoverPath || "", coverSource: runtimeCoverSource, coverUpdatedAt: song.coverUpdatedAt || null, durationMs: Number(song.durationMs || (Number(song.duration || 0) * 1000) || 0), savedCoverPath, savedCoverExists, usesFallbackCover: runtimeCoverSource === "fallback" || !savedCoverExists, missingSavedCover: !savedCoverPath || (Boolean(savedCoverPath) && !savedCoverExists), coverUrl, coverThumbUrl, coverThumbnailUrl: coverThumbUrl, thumbnailUrl: coverThumbUrl, coverFullUrl: coverUrl, exists, fileExists: exists, coverExists, coverThumbnailReady: Boolean(coverThumbUrl) };
}
...
function buildRandomizeMissingSongCovers() {
  ...
  const songs = getSongs();
  for (const song of songs.filter((song) => !song.coverPath || !fileExists(song.coverPath))) {
...
async function repairMissingSongMetadata(payload = {}) {
  ...
  const targets = allSongs.filter((song) => {
    ...
    const coverMissing = !song.coverPath || !fileExists(song.coverPath) || !song.coverSource || song.coverSource === "fallback" || song.coverSource === "none" || song.coverSource === "unknown";
  ...
  for (const song of targets) {
    ...
    const coverResult = await resolveSongCover({ song, metadata, filePath: song.filePath, folderCoverPath: findFolderCover(song.filePath), fallbackCoverPath: fallbackCover });
    const patch = {};
    ...
    const metadataDurationMs = Math.max(0, Number(metadata?.durationMs || 0), Number(metadata?.duration || 0) > 0 ? Number(metadata.duration) * 1000 : 0);
  ...
  clearFileInfoCache();
  return { ok: true, scannedCount: targets.length, changedCount, durationFixedCount, coverFixedCount, changedIds, songs: listSongsShaped() };
}

function analyzeVolumeGain(filePath) {
  return new Promise((resolve) => {
    if (!fileExists(filePath) || !isAudioFile(filePath)) { resolve({ ok: false, volumeGain: 1, error: "file missing or unsupported" }); return; }
    const ffmpegPath = getFfmpegPath();
    if (!ffmpegPath || !fileExists(ffmpegPath)) { resolve({ ok: false, volumeGain: 1, error: "ffmpeg missing" }); return; }
    execFile(ffmpegPath, ["-hide_banner", "-nostats", "-i", filePath, "-af", "volumedetect", "-f", "null", "-"], { windowsHide: true, timeout: 45000, maxBuffer: 1024 * 1024 * 6 }, (error, stdout, stderr) => {
      const text = `${stdout || ""}\n${stderr || ""}`;
      const mean = Number((text.match(/mean_volume:\s*(-?[0-9.]+) dB/i) || [])[1]);
      const max = Number((text.match(/max_volume:\s*(-?[0-9.]+) dB/i) || [])[1]);
      if (!Number.isFinite(mean)) { resolve({ ok: false, volumeGain: 1, error: error?.message || "could not read volume" }); return; }
      let gainDb = -18 - mean;
...
function openImportDialog(senderWindow) {
  const dialogOptions = { title: "Import local music files to localtify library", buttonLabel: "Add to Library", properties: ["openFile", "multiSelections"], filters: [{ name: "Audio Files", extensions: ["mp3", "wav", "ogg", "flac", "m4a", "aac"] }] };
  return senderWindow && !senderWindow.isDestroyed() ? dialog.showOpenDialog(senderWindow, dialogOptions) : dialog.showOpenDialog(dialogOptions);
}
...
function getRendererProtocolEntryUrl() { return `${LOCALTIFY_RENDERER_PROTOCOL}://app/index.html`; }
function isPackagedLinuxAppImageRuntime() { return Boolean(app.isPackaged && process.platform === "linux"); }
...
function readLocaltifyRendererFile(filePath) {
  return new Promise((resolve, reject) => fs.readFile(filePath, (error, data) => error ? reject(error) : resolve(data)));
}
function registerLocaltifyRendererProtocol() {
  ...
  protocol.handle(LOCALTIFY_RENDERER_PROTOCOL, async (request) => {
    ...
    const targetPath = requestPath === "index.html" ? indexPath : path.normalize(path.join(distDir, requestPath));
    ...
    if (!insideDist) return new Response("Blocked localtify renderer path", { status: 403 });
    ...
    try {
      ...
      return new Response(data, { status: 200, headers: { "content-type": getLocaltifyRendererMimeType(normalizedTarget), "cache-control": "no-cache" } });
    } catch (error) {
...
function loadPackagedLocaltifyRenderer(win, indexPath) {
  ...
  return win.loadURL(protocolUrl).catch((protocolError) => {
    ...
    if (isPackagedLinuxAppImageRuntime()) throw protocolError;
    return win.loadFile(indexPath);
  });
}

function getLocaltifyGpuFeatureStatus() {
  try { return typeof app.getGPUFeatureStatus === "function" ? app.getGPUFeatureStatus() : {}; } catch (error) { return { error: error?.message || String(error || "GPU feature status unavailable") }; }
}
async function getLocaltifyPerformanceStatus() {
  let gpuInfo = null;
  try { gpuInfo = typeof app.getGPUInfo === "function" ? await app.getGPUInfo("basic") : null; } catch (error) { gpuInfo = { error: error?.message || String(error || "GPU info unavailable") }; }
  let metrics = [];
  try { metrics = typeof app.getAppMetrics === "function" ? app.getAppMetrics().map((metric) => ({ type: metric.type, pid: metric.pid, cpu: metric.cpu, memory: metric.memory, sandboxed: metric.sandboxed })) : []; } catch { metrics = []; }
  const win = mainWindow && !mainWindow.isDestroyed() ? mainWindow : null;
  let devToolsOpened = false;
  try { devToolsOpened = Boolean(win?.webContents?.isDevToolsOpened?.()); } catch {}
  return { ok: true, appVersion: app.getVersion(), electronVersion: process.versions.electron, chromeVersion: process.versions.chrome, nodeVersion: process.versions.node, platform: process.platform, arch: process.arch, isPackaged: Boolean(app.isPackaged), gpuTuning: LOCALTIFY_CHROMIUM_PERFORMANCE, gpuFeatureStatus: getLocaltifyGpuFeatureStatus(), gpuInfo, processMetrics: metrics, window: { exists: Boolean(win), visible: Boolean(win?.isVisible?.()), focused: Boolean(win?.isFocused?.()), devToolsOpened } };
}
function openLocaltifyDevTools(win = mainWindow, options = {}) {
  try {
    ...
    const mode = typeof options?.mode === "string" && options.mode ? options.mode : (isDev ? "detach" : "right");
    target.webContents.openDevTools({ mode, activate: true });
    return { ok: true, opened: true, mode, gpuFeatureStatus: getLocaltifyGpuFeatureStatus() };
  } catch (error) { return { ok: false, error: error?.message || String(error || "failed to open DevTools") }; }
}
function toggleLocaltifyDevTools(win = mainWindow) {
  try {
    const target = win && !win.isDestroyed() ? win : mainWindow;
    if (!target || target.isDestroyed()) return { ok: false, error: "main window is not ready" };
    if (target.webContents.isDevToolsOpened()) { target.webContents.closeDevTools(); return { ok: true, opened: false }; }
    return openLocaltifyDevTools(target);
  } catch (error) { return { ok: false, error: error?.message || String(error || "failed to toggle DevTools") }; }
}
...
function localtifyStartupShellHtml(title, message, detail = "") {
  ...
  return `<!doctype html><html><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1" /><style>html,body{margin:0;width:100%;height:100%;background:#090012;color:#f8eaff;font-family:Inter,Segoe UI,system-ui,sans-serif;}body{display:grid;place-items:center;overflow:hidden;}.box{max-width:520px;padding:28px;border:1px solid rgba(232,94,255,.28);border-radius:26px;background:linear-gradient(135deg,rgba(42,8,56,.92),rgba(8,2,16,.96));box-shadow:0 24px 80px rgba(0,0,0,.55),0 0 42px rgba(220,76,255,.18);}h1{margin:0 0 10px;font-size:23px;letter-spacing:-.04em;}p{margin:0;color:#cbb8d8;line-height:1.55;font-size:14px;}code{display:block;margin-top:14px;padding:12px;border-radius:14px;background:rgba(255,255,255,.06);color:#f4d5ff;white-space:pre-wrap;word-break:break-word;font-size:12px;}</style></head><body><div class="box"><h1>${safe(title)}</h1><p>${safe(message)}</p>${detail ? `<code>${safe(detail)}</code>` : ""}</div></body></html>`;
}
function loadLocaltifyStartupShell(win, title, message, detail = "") {
  if (!win || win.isDestroyed()) return;
  try { win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(localtifyStartupShellHtml(title, message, detail))}`).catch(() => {}); } catch (error) { console.log("[localtify startup shell error]", error?.message || error); }
}
function hardenRendererBackground(win) {
  if (!win || win.isDestroyed()) return;
  try { win.webContents.insertCSS(`html, body, #root { min-width: 100%; min-height: 100%; background: #090012 !important; } body:empty::before { content: ""; position: fixed; inset: 0; background: #090012; }`).catch(() => {}); } catch {}
}
function verifyRendererMounted(win, indexPath) {
  ...
  setTimeout(async () => {
    if (!win || win.isDestroyed()) return;
    try {
      const state = await win.webContents.executeJavaScript(`(() => { const root=document.getElementById('root'); const body=document.body; return { href:location.href, rootChars:root?(root.innerHTML||'').trim().length:0, bodyChars:body?(body.innerHTML||'').trim().length:0, title:document.title||'' }; })()`, true);
      ...
      if (!mainWindowRendererRecoveredOnce) { mainWindowRendererRecoveredOnce = true; win.webContents.reloadIgnoringCache(); return; }
      loadLocaltifyStartupShell(win, "localtify could not finish opening", "The window started, but the renderer stayed empty. Restart Localtify once; if this repeats, run npm run build before packaging.", `Renderer: ${indexPath}`);
    } catch (error) { console.log("[localtify renderer verify error]", error?.message || error); }
  }, 2500);
}
function armRendererLoadWatchdog(win, indexPath) {
  ...
  mainWindowRendererWatchdog = setTimeout(() => {
    ...
    try {
      ...
      if (loading) { try { win.webContents.stop(); } catch {} }
      if (!mainWindowRendererRecoveredOnce) {
        mainWindowRendererRecoveredOnce = true;
        loadPackagedLocaltifyRenderer(win, indexPath).catch((error) => loadLocaltifyStartupShell(win, "localtify could not load", error?.message || "Renderer load failed.", indexPath));
        return;
      }
    ...
    } catch (error) { console.log("[localtify renderer watchdog error]", error?.message || error); }
  }, 14000);
}
function clearRendererLoadWatchdog() { clearTimeout(mainWindowRendererWatchdog); mainWindowRendererWatchdog = null; }
function attachLocaltifyDevToolsShortcuts(win) {
  ...
  win.webContents.on("before-input-event", (event, input = {}) => {
    ...
    if (!((isCtrlOrMeta && input.shift && key === "i") || key === "f12")) return;
    event.preventDefault();
    const result = toggleLocaltifyDevTools(win);
    if (!result?.ok) console.log("[localtify devtools shortcut error]", result?.error || result);
  });
}
...
function createWindow() {
  ...
  const nativeWindowOptions = process.platform === "win32" ? { backgroundMaterial: "none" } : {};
  ...
  installRendererSecurityGuards(mainWindow, { isDev, rendererFileRoot: rendererIndexPath ? path.dirname(rendererIndexPath) : "" });
  installSessionPermissionGuards(mainWindow.webContents?.session);
  mainWindow.webContents.on("dom-ready", () => hardenRendererBackground(mainWindow));
  ...
  if (isDev) mainWindow.loadURL("http://localhost:5173").catch((error) => console.log("[localitfy main window dev load error]", error?.message || error));
  else {
    console.log("[localitfy renderer index]", rendererIndexPath);
    ...
    loadPackagedLocaltifyRenderer(mainWindow, rendererIndexPath).catch((error) => {
      console.log("[localitfy main window packaged load error]", error?.message || error, rendererIndexPath);
  ...
  mainWindow.webContents.on("did-fail-load", (_e, code, desc, url, isMainFrame) => {
    ...
    const isIgnorableAbort = code === -3 && (failedUrl.startsWith("data:") || failedUrl.startsWith(`${LOCALTIFY_RENDERER_PROTOCOL}://`) || (isPackagedLinuxAppImageRuntime() && failedUrl.startsWith("file:")));
    if (!isDev && isMainFrame !== false && !isIgnorableAbort) loadLocaltifyStartupShell(mainWindow, "localtify failed to load", `${desc || "load failed"} (${code})`, url || rendererIndexPath);
  });
  mainWindow.webContents.on("render-process-gone", (_e, details) => {
    ...
    if (!isDev && !mainWindowRendererRecoveredOnce && mainWindow && !mainWindow.isDestroyed()) {
      mainWindowRendererRecoveredOnce = true;
      setTimeout(() => { try { void loadPackagedLocaltifyRenderer(mainWindow, rendererIndexPath); } catch {} }, 450);
    }
  });
  mainWindow.webContents.on("console-message", (_event, ...args) => {
    const details = args.length === 1 && args[0] && typeof args[0] === "object" ? args[0] : { level: args[0], message: args[1], line: args[2], sourceId: args[3] };
    console.log("[localitfy main window console]", { level: details.level, message: details.message, line: details.line, sourceId: details.sourceId });
  });
  if (process.env.LOCALITFY_DEBUG === "1") openLocaltifyDevTools(mainWindow, { mode: "detach" });
  mainWindow.once("ready-to-show", () => { repairMainWindowBounds(mainWindow, { center: true }); mainWindow.show(); });
  if (isPackagedLinuxAppImageRuntime()) {
    setTimeout(() => {
      try {
        if (mainWindow && !mainWindow.isDestroyed() && !mainWindow.isVisible()) { repairMainWindowBounds(mainWindow, { center: true }); mainWindow.show(); }
      } catch (error) { console.log("[localtify linux delayed show error]", error?.message || error); }
    }, 1600);
  }
  mainWindow.on("closed", () => { clearRendererLoadWatchdog(); mainWindow = null; });
...
function getSafeMainWindowSize() {
  ...
  return { width: Math.min(MAIN_WINDOW_DEFAULT_WIDTH, Math.max(MAIN_WINDOW_MIN_WIDTH, Math.floor(workArea.width * 0.94))), height: Math.min(MAIN_WINDOW_DEFAULT_HEIGHT, Math.max(MAIN_WINDOW_MIN_HEIGHT, Math.floor(workArea.height * 0.9))) };
}
function repairMainWindowBounds(win, options = {}) {
  ...
  const width = Math.min(Math.max(current.width, minWidth), maxWidth);
  const height = Math.min(Math.max(current.height, minHeight), maxHeight);
  let x = current.x;
  let y = current.y;
...
function getPackagedEnvCandidatePaths() {
  const safePath = (...parts) => { try { return path.join(...parts.filter(Boolean)); } catch { return ""; } };
  let cwd = ""; try { cwd = process.cwd(); } catch {}
  let resourcePath = ""; try { resourcePath = process.resourcesPath || ""; } catch {}
  let executableDir = ""; try { executableDir = process.execPath ? path.dirname(process.execPath) : ""; } catch {}
  let userDataPath = ""; try { userDataPath = typeof app.getPath === "function" ? app.getPath("userData") : ""; } catch {}
  return [safePath(cwd, ".env"), safePath(cwd, ".env.production"), safePath(executableDir, ".env"), safePath(executableDir, ".env.production"), safePath(resourcePath, ".env"), safePath(resourcePath, ".env.production"), safePath(resourcePath, "app", ".env"), safePath(resourcePath, "app.asar.unpacked", ".env"), safePath(userDataPath, ".env"), safePath(userDataPath, ".env.production")].filter(Boolean);
}
function getFeedbackWebhookInfo() {
  const keys = ["LOCALTIFY_FEEDBACK_WEBHOOK_URL", "LOCALITFY_FEEDBACK_WEBHOOK_URL", "VITE_LOCALTIFY_FEEDBACK_WEBHOOK_URL", "VITE_LOCALITFY_FEEDBACK_WEBHOOK_URL", "FEEDBACK_WEBHOOK_URL"];
  for (const key of keys) {
    const value = String(process.env[key] || "").trim();
    if (!value) continue;
    const valid = /^https:\/\/(?:discord(?:app)?\.com)\/api\/webhooks\/\d+\/[A-Za-z0-9._-]+(?:\?.*)?$/.test(value);
    return { configured: true, valid, envName: key, url: value, label: valid ? "Discord feedback enabled" : "Discord webhook invalid", message: valid ? "Feedback will be delivered to your Discord channel." : "Use a real https://discord.com/api/webhooks/... URL." };
  }
  return { configured: false, valid: false, envName: "", url: "", label: "Discord feedback not configured", message: "Set LOCALTIFY_FEEDBACK_WEBHOOK_URL in .env, beside the exe, resources, or userData." };
}
function getFeedbackStatus() {
  const info = getFeedbackWebhookInfo();
  return { configured: Boolean(info.configured), valid: Boolean(info.valid), envName: info.envName || "", label: info.label, message: info.message, isPackaged: Boolean(app.isPackaged) };
}
...
function postDiscordWebhookWithElectronNet(webhookUrl, body) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      ...
      done({ ok: false, code: "webhook_timeout", error: "Discord webhook request timed out. Check internet/firewall or regenerate the webhook URL. Check the webhook URL, internet connection, or Discord reachability, then try again." });
    }, 30_000);
    try {
      request = net.request({ method: "POST", url: webhookUrl, redirect: "follow" });
      ...
      request.on("response", (response) => {
        let raw = "";
        response.on("data", (chunk) => { raw += Buffer.isBuffer(chunk) ? chunk.toString("utf8") : String(chunk || ""); });
        response.on("end", () => {
          const ok = response.statusCode >= 200 && response.statusCode < 300;
          done({ ok, statusCode: response.statusCode, code: ok ? "ok" : "webhook_failed", error: ok ? "" : raw.slice(0, 500) || `Discord returned HTTP ${response.statusCode}` });
        });
      });
      request.on("error", (error) => done({ ok: false, code: "webhook_failed", error: error?.message || "Discord webhook request failed." }));
      request.write(payload);
      request.end();
    } catch (error) { done({ ok: false, code: "webhook_failed", error: error?.message || "Discord webhook request failed before sending." }); }
  });
}
...
function postDiscordWebhookWithNodeHttps(webhookUrl, body) {
  return new Promise((resolve) => {
    ...
    try { parsed = new URL(webhookUrl); } catch { done({ ok: false, code: "webhook_invalid", error: "Webhook URL could not be parsed." }); return; }
    const payload = Buffer.from(JSON.stringify(body), "utf8");
    const request = https.request({ method: "POST", hostname: parsed.hostname, path: `${parsed.pathname}${parsed.search || ""}`, headers: { "Content-Type": "application/json", "Content-Length": String(payload.length), "User-Agent": "Localtify-Feedback/0.4.1" }, timeout: 30_000 }, (response) => {
      let raw = "";
    ...
    request.on("timeout", () => request.destroy(new Error("Discord webhook request timed out. Check internet/firewall or regenerate the webhook URL.")));
    request.on("error", (error) => done({ ok: false, code: error?.message?.toLowerCase().includes("timed out") ? "webhook_timeout" : "webhook_failed", error: error?.message || "Discord webhook request failed." }));
    request.write(payload);
    request.end();
  });
}
...
async function postDiscordWebhook(webhookUrl, body) {
  if (net && typeof net.request === "function") {
    ...
    if (electronResult.ok || electronResult.code === "webhook_timeout") return electronResult;
  }
...
async function sendLocaltifyFeedback(payload = {}) {
  ...
  if (!webhook.configured) return { ok: false, code: "webhook_missing", error: webhook.message };
  if (!webhook.valid) return { ok: false, code: "webhook_invalid", error: webhook.message };
  ...
  const diagnostics = payload && typeof payload.diagnostics === "object" && payload.diagnostics ? payload.diagnostics : {};
  if (message.length < 4) return { ok: false, code: "message_too_short", error: "Feedback message is too short." };
  const fieldFrom = (name, value) => ({ name, value: cleanFeedbackText(value, 220) || "unknown", inline: true });
  const fields = [fieldFrom("category", category), fieldFrom("app", appVersion || "unknown"), fieldFrom("platform", platform || process.platform), fieldFrom("songs", diagnostics.songCount), fieldFrom("playlists", diagnostics.playlistCount), fieldFrom("downloads folder", diagnostics.downloadsFolder), fieldFrom("discord rpc", diagnostics.discordRpc), fieldFrom("update status", diagnostics.updateStatus)].filter((field) => field.value !== "unknown" || ["category", "app", "platform"].includes(field.name));
  const result = await postDiscordWebhook(webhook.url, { username: "Localtify Feedback", avatar_url: "https://raw.githubusercontent.com/meshahid973/localitfy/main/build/icon.png", allowed_mentions: { parse: [] }, embeds: [{ title: `Localtify feedback — ${category}`, description: message, color: 0xd946ef, timestamp: new Date().toISOString(), fields }] });
  if (!result.ok) return { ok: false, code: result.code || "webhook_failed", statusCode: result.statusCode, error: result.error || "Discord did not accept the feedback." };
  return { ok: true, code: "sent", statusCode: result.statusCode, envName: webhook.envName };
}
...
app.whenReady().then(async () => {
  ...
  ipcRouter.handle("localitfy:open-devtools", async (event, payload = {}) => openLocaltifyDevTools(BrowserWindow.fromWebContents(event.sender) || mainWindow, payload));
  ipcRouter.handle("localitfy:toggle-devtools", async (event) => toggleLocaltifyDevTools(BrowserWindow.fromWebContents(event.sender) || mainWindow));
  ...
  ipcRouter.handle("playback:resolve-url", async (_event, payload = {}) => {
    try {
      ...
      if (!filePath || !path.isAbsolute(filePath)) return { ok: false, fileExists: false, error: "invalid audio file path" };
      if (!isAudioFile(filePath)) return { ok: false, filePath, fileExists: fileExists(filePath), error: "unsupported audio file type" };
      if (!fileExists(filePath)) return { ok: false, filePath, fileExists: false, error: "audio file not found" };
      const serverReady = await startLocaltifyMediaServer();
      if (!serverReady || !mediaServerRuntime.getPort()) return { ok: false, filePath, fileExists: true, error: "local media server unavailable" };
      const versionInfo = getMediaFileVersion(filePath);
      return { ok: true, filePath, url: safeMediaUrl(filePath), fileExists: true, sizeBytes: versionInfo.sizeBytes, mtimeMs: versionInfo.mtimeMs, cacheTtlMs: MEDIA_TOKEN_TTL_MS };
    } catch (error) {
  ...
  ipcRouter.handle("covers:list-pixelart", async () => { try { return listPixelCoversDetailed(); } catch (e) { console.log("[localitfy list covers error]", e?.message); return []; } });
  ipcRouter.handle("covers:stats", async () => { try { return getCoverStats(); } catch (e) { console.log("[localitfy covers stats error]", e?.message); return {}; } });
  ipcRouter.handle("covers:rescan", async () => { clearPixelArtCache(); clearFileInfoCache(); await yieldToMainLoop(); return buildRandomizeMissingSongCovers(); });
  ...
  ipcRouter.handle("covers:randomize-missing", async () => { await yieldToMainLoop(); return buildRandomizeMissingSongCovers(); });
  ...
  ipcRouter.handle("covers:broken", async () => { try { return getCoverStats().brokenItems || []; } catch { return []; } });
  ipcRouter.handle("covers:least-used", async () => { try { return pickLeastUsedCover(getPixelArtFiles()); } catch { return ""; } });
  ipcRouter.handle("covers:thumbnail-status", async () => { try { return getCoverThumbnailStatus(); } catch (error) { return { ok: false, error: error?.message || "thumbnail status failed" }; } });
  ipcRouter.handle("covers:warm-thumbnails", async (_event, payload = {}) => {
    try {
      await yieldToMainLoop();
      return warmCoverThumbnails({ limit: Math.max(1, Math.min(500, Number(payload?.limit || 120))), force: Boolean(payload?.force) });
    } catch (error) {
      console.log("[localtify warm thumbnails error]", error?.message || error);
      return { ok: false, created: 0, cached: 0, warnings: ["Localtify could not prepare cover thumbnails right now. Covers will still load normally."], error: error?.message || "thumbnail warmup failed" };
    }
  });
  ipcRouter.handle("covers:cleanup-cache", async () => { try { return cleanupCoverThumbnailCache(); } catch (error) { return { ok: false, error: error?.message || "cover cache cleanup failed" }; } });
  ipcRouter.handle("song:set-cover", async (_event, id, coverPath) => { const updated = patchSong(id, { coverPath }); return updated ? shapeSong(updated) : null; });
  ipcRouter.handle("song:pick-cover", async (event, id) => {
    const song = getSongs().find((item) => item.id === id);
    if (!song) return null;
    try {
      const senderWindow = BrowserWindow.fromWebContents(event.sender) || mainWindow;
      const options = { title: "Choose localtify cover image", buttonLabel: "Use cover", properties: ["openFile"], filters: [{ name: "Images", extensions: ["png", "jpg", "jpeg", "webp", "gif", "bmp"] }] };
      const result = senderWindow && !senderWindow.isDestroyed() ? await dialog.showOpenDialog(senderWindow, options) : await dialog.showOpenDialog(options);
      ...
      console.log("[localitfy pick cover error]", error?.message || error);
  ...
  ipcRouter.handle("song:analyze-volume", async (_event, id) => {
    ...
    return { ...analysis, song: updated ? shapeSong(updated) : shapeSong(target) };
  });
  ipcRouter.handle("library:import", async (event) => {
    try {
      ...
      return afterSongs;
    } catch (error) { console.log("[localitfy import error]", error?.stack || error?.message); return listSongsShaped(); }
  });
  ipcRouter.handle("library:repair-missing-metadata", async (_event, payload = {}) => { try { return await repairMissingSongMetadata(payload); } catch (error) { console.log("[localtify metadata repair error]", error?.stack || error?.message || error); return { ok: false, error: error?.message || "metadata repair failed", songs: listSongsShaped() }; } });
  ipcRouter.handle("album:scan-folder", async (event, payload = {}) => { try { return await handleAlbumFolderScan(event, payload); } catch (error) { console.log("[localtify album folder scan error]", error?.stack || error?.message || error); return { ok: false, error: error?.message || "album folder scan failed", songs: listSongsShaped(), albums: [] }; } });
  ipcRouter.handle("album:import-folder", async (event, payload = {}) => { try { return await handleAlbumFolderImport(event, payload); } catch (error) { console.log("[localtify album folder import error]", error?.stack || error?.message || error); return { ok: false, error: error?.message || "album folder import failed", songs: listSongsShaped(), albums: [] }; } });

  ipcRouter.handle("media:convert-pick", async (event, payload) => {
    try {
      const senderWindow = BrowserWindow.fromWebContents(event.sender) || mainWindow;
      const dialogOptions = { title: "choose videos or audio to convert", buttonLabel: "convert to mp3", defaultPath: app.getPath("downloads"), properties: ["openFile", "multiSelections"], filters: [{ name: "media files", extensions: ["mp4", "webm", "mkv", "mov", "avi", "m4v", "mp3", "wav", "ogg", "flac", "m4a", "aac"] }] };
      const result = senderWindow && !senderWindow.isDestroyed() ? await dialog.showOpenDialog(senderWindow, dialogOptions) : await dialog.showOpenDialog(dialogOptions);
      if (result.canceled || !result.filePaths.length) return { downloadFolder: getDownloadDirectory(), conversions: [], changedCount: 0, songs: listSongsShaped() };
      const validFiles = result.filePaths.filter((f) => fileExists(f) && isSupportedMediaPath(f));
      const converted = await convertLocalMediaFiles(validFiles, getDownloadDirectory(), { bitrate: payload?.bitrate || 192 }, (progress) => { event.sender.send("download:progress", progress); });
      const successfulFiles = converted.conversions.filter((item) => item.ok && item.filePath && fileExists(item.filePath) && isAudioFile(item.filePath)).map((item) => item.filePath);
      let changedCount = 0;
      let afterSongs = listSongsShaped();
      if (successfulFiles.length) {
        const pixelArtFiles = getPixelArtFiles();
        const usedCovers = new Set();
        changedCount = insertSongs(await Promise.all(successfulFiles.map((f) => makeSongFromFile(f, pixelArtFiles, usedCovers))));
        clearFileInfoCache();
        afterSongs = listSongsShaped();
      }
      return { ...converted, changedCount, songs: afterSongs, downloadFolder: getDownloadDirectory() };
    } catch (error) { console.log("[localitfy convert pick error]", error?.message); return { downloadFolder: getDownloadDirectory(), conversions: [], changedCount: 0, songs: listSongsShaped() }; }
  });
  ...
  ipcRouter.handle("download:audio", async (event, payload) => {
    try {
      ...
      const successfulFiles = result.downloads.filter((item) => item.ok && item.filePath && fileExists(item.filePath) && isAudioFile(item.filePath)).map((item) => item.filePath);
      let changedCount = 0;
      let afterSongs = listSongsShaped();
      ...
      return { downloadFolder: getDownloadDirectory(), downloads: [{ ok: false, error: error?.message || "download failed" }], changedCount: 0, songs: listSongsShaped(), autoAdd: true };
    }
  });
  ...
  ipcRouter.handle("download:open-folder", async (_event, folder) => { await shell.openPath(getDownloadDirectory(folder)); return true; });
  ipcRouter.handle("song:patch", async (_event, id, patch) => { const updated = patchSong(id, patch); return updated ? shapeSong(updated) : null; });
  ...
  ipcRouter.handle("song:delete", async (_event, id) => { try { deleteSong(id); return listSongsShaped(); } catch (error) { console.log("[localitfy delete song error]", error?.message); return listSongsShaped(); } });
  ipcRouter.handle("library:clear", async () => { clearLibrary(); return []; });
  ipcRouter.handle("library:refresh-download-folder", async (_event, payload = {}) => {
    ...
    return { ok: true, downloadFolder: target, changedCount: refresh.changedCount + repair.changedCount, importedCount: refresh.importedCount, repairedCount: repair.changedCount, songs: repair.songs || refresh.songs };
  });
  ...
  ipcRouter.handle("settings:save", async (_event, settings) => {
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(settings, "startWithWindows")) setStartWithWindows(Boolean(settings.startWithWindows));
    ...
    } catch (error) { console.log("[localitfy settings save error]", error?.message); return getSettings(); }
  });
  ...
  ipcRouter.handle("localitfy:set-window-translucency", async (_event, payload = {}) => {
    try {
      ...
      saveSettings({ translucentWindow: next.translucentWindow, windowTransparency: next.windowTransparency, windowBlur: next.windowBlur, transparentAppBackground: next.transparentAppBackground });
      if (mainWindow && !mainWindow.isDestroyed()) applyWindowTranslucencyToWindow(mainWindow, next);
      return { ok: true, restartRequired: false, windowReloadRequired: false, changedNativeMaterial: transparentModeChanged, ...next };
    } catch (error) { console.log("[localitfy translucent window save error]", error?.message || error); return { ok: false, error: error?.message || "could not update translucent window", ...getSavedWindowTranslucencySettings() }; }
  });
  ipcRouter.handle("localitfy:restart-app", async () => { restartForWindowTranslucency(); return true; });
  ipcRouter.handle("localitfy:open-logs", async () => {
    ...
    } catch (error) { return { ok: false, error: error?.message || "could not open logs folder" }; }
  });
  ipcRouter.handle("playlists:get", async () => getPlaylists());
  ipcRouter.handle("playlists:save", async (_event, playlists) => { try { return savePlaylists(playlists); } catch { return getPlaylists(); } });
  ipcRouter.handle("database:backup-now", async () => { try { return { ok: backupDatabase() }; } catch (e) { return { ok: false, error: e?.message }; } });
  ipcRouter.handle("database:repair-now", async () => {
    try {
      repairDatabaseNow();
      mediaServerRuntime.clearTokens();
  ...
  registerDiscordIpc(ipcRouter, { setDiscordActivity, clearDiscordActivity, getDiscordStatus, resetDiscordActivityCache });
  ...
  const runSpotifyFetch = async (payload = {}) => {
    try {
      ...
      console.log(`[localitfy spotify] fetched ${result.tracks.length} public track(s) from ${result.type} "${result.name}"`, {
      ...
      return { ok: false, publicOnly: true, tracks: [], error: error?.message || "Failed to fetch Spotify tracks.", hint: "Spotify has two visibility states: shareable by link and public on profile. If fetch fails, open the playlist menu in Spotify and make it public/add it to profile." };
    }
  };
  ...
  ipcRouter.handle("spotify-check", async () => {
    ...
    return { ok: true, ready: true, loggedIn: Boolean(saved?.refreshToken), publicOnly: true, fallbackAvailable: true, mode: hasClientId ? "oauth-pkce" : "public-fallback", needsClientId: false, redirectUri: SPOTIFY_REDIRECT_URI, error: "" };
  });
  ipcRouter.handle("spotify-login", async () => loginSpotifyOAuth());
  ipcRouter.handle("spotify-import-browser", async () => ({ ok: false, ready: true, loggedIn: Boolean(readSpotifyOAuthToken()?.refreshToken), publicOnly: true, fallbackAvailable: true, mode: SPOTIFY_CLIENT_ID ? "oauth-pkce" : "public-fallback", needsClientId: false, redirectUri: SPOTIFY_REDIRECT_URI, error: "Browser-cookie Spotify import was removed. Use Connect Spotify, or paste a public Spotify link directly." }));
  ipcRouter.handle("spotify-set-cookie", async () => ({ ok: false, ready: true, loggedIn: Boolean(readSpotifyOAuthToken()?.refreshToken), publicOnly: true, fallbackAvailable: true, mode: SPOTIFY_CLIENT_ID ? "oauth-pkce" : "public-fallback", needsClientId: false, redirectUri: SPOTIFY_REDIRECT_URI, error: "Cookie login was removed. Use Connect Spotify, or paste a public Spotify link directly." }));
  ipcRouter.handle("spotify-logout", async () => { clearSpotifyOAuthToken(); console.log("[localitfy spotify] oauth token cleared"); return { ok: true, ready: true, loggedIn: false, publicOnly: true, fallbackAvailable: true, mode: SPOTIFY_CLIENT_ID ? "oauth-pkce" : "public-fallback", needsClientId: false }; });
  ipcRouter.handle("spotdl-check", async () => ({ ok: true, installed: true, engine: "localtify-ytdlp-bridge", message: "Spotify downloads use localtify's bundled yt-dlp bridge after metadata is fetched." }));
  ...
  const runSpotifyDownloadBatch = async (event, payload = {}) => {
    ...
    try {
      ...
      const progressCallback = (progress) => {
        if (!mainWindow || mainWindow.isDestroyed()) return;
        event.sender.send("download:progress", progress);
        if (progress?.status === "done") event.sender.send("spotdl-track-done", progress);
      };
      const result = await downloadSpotifyBatch(tracks, downloadFolder, progressCallback, options);
      const tracksBySpotifyId = new Map(tracks.map((track) => [String(track?.spotifyTrackId || track?.id || "").trim(), track]).filter(([id]) => Boolean(id)));
      const successfulDownloads = (result.downloads || []).map((item, index) => {
        const spotifyTrackId = String(item?.spotifyTrackId || "").trim();
        const track = tracksBySpotifyId.get(spotifyTrackId) || tracks[index] || (item?.filePath ? matchSpotifyTrackForFile(item.filePath, tracks) : null) || {};
        return { item, track };
      }).filter(({ item }) => item?.ok && item.filePath && item.matchOk !== false && fileExists(item.filePath) && isAudioFile(item.filePath));
      let changedCount = 0;
      let afterSongs = listSongsShaped();
      ...
      if (autoAdd && successfulDownloads.length) {
        ...
        for (const { item, track } of successfulDownloads) {
          ...
          const sourceInfo = { sourceTrackId: spotifyTrackId, sourceUrl: String(item.spotifyUrl || track?.spotifyUrl || (spotifyTrackId ? `https://open.spotify.com/track/${spotifyTrackId}` : "")).trim(), sourceProvider: String(item.provider || "youtube"), sourceProviderUrl: String(item.providerUrl || ""), sourceMatchScore: Number(item.matchScore || 0) };
          ...
          spotifyImportMap.push({ spotifyTrackId, songId: song.id, filePath });
        }
        changedCount = insertSongs(importedSongs);
        for (const song of importedSongs) {
          try {
            patchSong(song.id, { title: song.title, artist: song.artist, album: song.album, coverPath: song.coverPath, duration: song.duration, sourceType: song.sourceType, sourceTrackId: song.sourceTrackId, sourceUrl: song.sourceUrl, sourceProvider: song.sourceProvider, sourceProviderUrl: song.sourceProviderUrl, sourceMatchScore: song.sourceMatchScore });
          } catch (error) { console.log("[localitfy spotify metadata patch error]", error?.message || error); }
        }
        clearFileInfoCache();
        afterSongs = listSongsShaped();
      }
      const importMapBySpotifyId = new Map(spotifyImportMap.map((item) => [String(item.spotifyTrackId || "").trim(), item]).filter(([id]) => Boolean(id)));
      const importMapByFilePath = new Map(spotifyImportMap.map((item) => [path.normalize(String(item.filePath || "")).toLowerCase(), item]).filter(([filePath]) => Boolean(filePath)));
      const finalDownloads = (result.downloads || []).map((item) => {
        const imported = importMapBySpotifyId.get(String(item?.spotifyTrackId || "").trim()) || (item?.filePath ? importMapByFilePath.get(path.normalize(String(item.filePath)).toLowerCase()) : null) || null;
        return { ...item, importedToLibrary: Boolean(imported), librarySongId: imported?.songId || item.librarySongId || "" };
      });
      for (const item of finalDownloads) if (item?.ok) event.sender.send("spotdl-track-done", item);
      return { ...result, downloads: finalDownloads, changedCount, songs: afterSongs, downloadFolder, importedFilePaths: Array.from(new Set(importedFilePaths.filter(Boolean))), spotifyImportedSongIds: Array.from(new Set(importedSongIds.filter(Boolean))), spotifyImportMap, spotifySourceName: payload?.sourceName || options?.sourceName || "", spotifySourceType: payload?.sourceType || options?.sourceType || "" };
    } catch (error) {
      console.log("[localitfy spotify batch download error]", error?.message || error);
      return { downloadFolder: getDownloadDirectory(options?.downloadFolder || options?.outputDir || options?.folder), downloads: [{ ok: false, error: error?.message || "batch download failed" }], changedCount: 0, songs: listSongsShaped(), spotifyImportedSongIds: [], spotifyImportMap: [], importedFilePaths: [] };
    }
  };
  ...
  ipcRouter.handle("window:minimize", async (event) => { BrowserWindow.fromWebContents(event.sender)?.minimize(); return true; });
  ...
  ipcRouter.handle("window:close", async (event) => { BrowserWindow.fromWebContents(event.sender)?.close(); return true; });

  const startupCreateDelayMs = process.platform === "win32" && startupLaunchStatus?.wasOpenedAtLogin ? 1600 : 0;
  setTimeout(() => { createWindow(); setupNativeWindowsMedia(); }, startupCreateDelayMs);
  setTimeout(() => { checkForUpdates({ silent: true }).catch((e) => console.log("[localitfy updater startup error]", e?.message)); }, UPDATE_CHECK_STARTUP_DELAY_MS);
  app.on("activate", () => { if (BrowserWindow.getAllWindows().length === 0) createWindow(); });
});

app.on("before-quit", async () => {
  allowQuit = true;
  if (serviceRuntime) await serviceRuntime.stop("app-before-quit");
  else {
