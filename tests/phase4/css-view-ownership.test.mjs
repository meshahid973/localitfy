...
test("renderer style manifest stays global-only during page rebuild", () => {
  const main = read("src/main.tsx");
  const cssImports = [...main.matchAll(/^\s*import\s+["']([^"']+\.css)["'];?\s*$/gm)].map((match) => match[1]);

  for (const specifier of ["./shared/ui/view-ui.css", "./styles/page-foundation.css", "./styles/themes.css", "./features/shell/performance.css"]) {
...
test("Library has graduated from reset mode to co-located ownership", () => {
  const view = read("src/features/library/LibraryView.tsx");
  assert.equal(fs.existsSync(path.join(root, "src/features/library/library.css")), true);
  assert.equal(view.includes('import "./library.css";'), true);
  assert.equal(view.includes('data-page-section="library"'), true);
  assert.equal(view.includes('data-page-state="reset"'), false);
});

test("Home remains isolated from future page redesigns", () => {
  const home = read("src/features/home/home.css");
  const forbidden = [".libraryPage", ".albumsPageV318", ".playlistsPage", ".coverStudioLayout", ".downloadsLayoutV031", ".analyticsStudioV339", ".localtifyStateCardV373"];
  for (const selector of forbidden) assert.equal(home.includes(selector), false, `${selector} must not drift into home.css`);
});
